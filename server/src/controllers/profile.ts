...
export const getAutoGeneratedPlaylist: RequestHandler = async (req, res) => {
  ...
  const finalList = autoGenPlaylist.concat({
    id: playlist?._id,
...
export const getPlaylistAudios: RequestHandler = async (req, res) => {
  ...
  if (!playlistResult) {
    const [autoGenPlaylistResult] = await AutoGeneratedPlaylist.aggregate(
      aggregationLogic
    );

    return res.json({ list: autoGenPlaylistResult });
  }
